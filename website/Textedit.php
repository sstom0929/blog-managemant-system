...
<body>
	...
	<?php
		...
		if(isset($_POST["submit"])){
            ...
            if(chop($tmpstr) != "" && chop($tmpstr2) != ""){
                $query="INSERT INTO article (title,class,content) VALUES ('".$_POST["title"]."','".$_POST["class"]."','".$_POST["content"]."')";
                ...
                if(chop($tmpstr2) == ""){
                    ...
                    echo '<script>document.location.href="article.php?center_display=new_article";</script>';
                }
                if(chop($tmpstr) == ""){
	...
	?>
    <!--<div id=body  style="background-color:#f2f2f2">
		<div style="height: 100%;">-->
			<p class='p_text'>標題</p>
			<input type="text" id="text_title" style="width:240px; height: 1.2em; border: 1.5px solid #000000;">
			<p class='p_text'>分類</p>
			<select style="height:24px; width:80px"id="text_class">
			...
			<div>
				<form action="article.php?center_display=new_article" method="post">
					<input id="submit" type="submit" name="submit" value="送出">
					<input id="title" type="hidden" name="title">
					<input id="class" type="hidden" name="class">
					<input id="content" type="hidden" name="content">
					<script>
						$(document).ready(function(){
							$("#submit").click(function(){
								$("#title").val($("#text_title").val());
								$("#class").val($("#text_class").val());	
