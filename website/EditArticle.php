	...
	<?php		
		...
		if(isset($_POST["submit"])){
			...
			if(chop($tmpstr) != "" && chop($tmpstr2) != ""){
                $query="UPDATE article SET title='".$_POST["title"]."',class='".$_POST["class"]."',content='".$_POST["content"]."' WHERE id=".$id;
	...
	<select style="height:24px; width:80px" id="text_class">
		...
		<?php
            ...
            function display_option($nowrow,$nowpre){
                ...
                if($nowrow['id']==$class){
                	print('<option value="'. $nowrow['id'] .'" selected>');
                }
                else{
